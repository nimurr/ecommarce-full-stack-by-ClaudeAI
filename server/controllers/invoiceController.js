...
export const generateInvoice = asyncHandler(async (req, res) => {
  ...
  doc.fillColor(WHITE).fontSize(26).font('Helvetica-Bold').text('INVOICE', margin, 28);
  doc.fontSize(11).font('Helvetica').text('Gadgets Lagbe', margin, 58);
  doc.fontSize(9).fillColor(GRAY).text('Dhaka, Bangladesh | 01852219894', margin, 74);

  doc.fillColor(PRIMARY).fontSize(14).font('Helvetica-Bold')
    .text(`#${order.orderNumber}`, margin + 300, 28, { align: 'right', width: contentWidth - 300 });

  doc.fillColor(GRAY).fontSize(10)
    .text(`Date: ${new Date(order.createdAt).toLocaleDateString('en-BD', { day: 'numeric', month: 'short', year: 'numeric' })}`, margin + 300, 50, { align: 'right', width: contentWidth - 300 });

  doc.text(`Status: ${order.orderStatus}`, margin + 300, 68, { align: 'right', width: contentWidth - 300 });
  ...
  doc.fillColor(DARK).fontSize(9).font('Helvetica-Bold').text('BILL TO', margin + 10, y + 5);

  doc.fontSize(11).text(order.shippingAddress?.fullName || order.user?.name || 'Guest', margin + 10, y + 30);

  doc.fillColor(GRAY).fontSize(9)
    .text(`Phone: ${order.shippingAddress?.phone || order.user?.phone || 'N/A'}`, margin + 10, y + 46);

  doc.text(`Email: ${order.user?.email || 'N/A'}`, margin + 10, y + 60);
  ...
  doc.fillColor(DARK).fontSize(9).font('Helvetica-Bold').text('PAYMENT INFO', paymentX + 10, y + 5);
  ...
  if (order.couponCode) {
    doc.fillColor(GREEN).text(`Coupon: ${order.couponCode}`, paymentX + 10, y + 60);
  }
  ...
  doc.fillColor(DARK).fontSize(9).font('Helvetica-Bold').text('SHIPPING ADDRESS', margin + 10, y + 4);
  ...
  // ---- COURIER ORDER ID (HANDWRITTEN FIELD) ----
  y += 70;
  ...
  doc.fillColor(DARK).fontSize(9).font('Helvetica-Bold')
    .text('COURIER ORDER ID (Steadfast)', margin + 10, y + 4);

  doc.moveTo(margin + 10, y + 35)
    .lineTo(margin + contentWidth - 10, y + 35)
    .lineWidth(1)
    .strokeColor(GRAY)
    .stroke();
  ...
  order.orderItems.forEach((item, index) => {
    ...
    if (item.selectedColor || item.selectedSize) {
      ...
      if (item.selectedColor) variantText += `Color: ${item.selectedColor}`;
      if (item.selectedSize) variantText += `${variantText ? ' | ' : ''}Size: ${item.selectedSize}`;
      doc.fontSize(7).fillColor(GRAY).text(variantText, margin + 35, y + 20);
      doc.fontSize(9).fillColor(DARK);
    }
  ...
  doc.fillColor(DARK)
    .text(`Tk ${(order.itemsPrice || 0).toLocaleString()}`, totalsX + 100, y, { width: 100, align: 'right' });

  y += 20;

  doc.fillColor(GRAY)
    .text('Shipping', totalsX, y, { width: 100, align: 'right' });

  doc.fillColor(DARK)
    .text(`Tk ${(order.shippingPrice || 0).toLocaleString()}`, totalsX + 100, y, { width: 100, align: 'right' });

  y += 20;

  if (order.discountPrice > 0) {
    doc.fillColor(GREEN)
      .text('Discount', totalsX, y, { width: 100, align: 'right' });
  ...
  doc.fillColor(WHITE).fontSize(12).font('Helvetica-Bold')
    .text('TOTAL', totalsX, y + 7, { width: 100, align: 'right' });
  ...
  doc.fillColor(GRAY).fontSize(9)
    .text('Thank you for your purchase!', margin, y, { align: 'center', width: contentWidth });
  ...
  doc.fontSize(7).fillColor('#94a3b8')
    .text(
      `Invoice generated on ${new Date().toLocaleDateString('en-BD')} at ${new Date().toLocaleTimeString('en-BD')}`,
      margin,
      y,
